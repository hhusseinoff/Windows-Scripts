...
## Last Edited: June 30th, 2023
## Function: Checks for installed SQL Server products (including Express, Standard, and LocalDB) and SSMS.
## Function: Returns 1 if any SQL server products are detected that are not compatible with Server 2022
...
$LogFolderPath = "C:\Temp\SQL_Checker2022"
...
if ($localDBVersions) {
    foreach ($localDBVersion in $localDBVersions.PSChildName) {
        ...
        if ($localDBVersion.Split(".")[0] -lt 14) {
...
if ($compatibilityIssues) {
    ...
    if ($instances.PSChildName -and $instanceVersion -lt 14) {
